...
class EthereumRPC {
    ...
    /**
     * Set callback for network activity logging
     * @param {Function} callback - Callback function (provider, method, status, data)
     */
    ...
    /**
     * Log network activity if callback is set
     * @param {string} provider - RPC provider URL
     * @param {string} method - RPC method
     * @param {string} status - Request status
     * @param {Object} data - Additional data
     */
    logActivity(provider, method, status, data = null) {
        if (this.activityCallback) {
            try {
                this.activityCallback(provider, method, status, data);
            } catch (error) {
    ...
    async call(method, params = []) {
        ...
        try {
            ...
            this.logActivity(provider, method, 'pending', { params });
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    ...
                    errorMsg = 'Unauthorized - API key invalid';
                    throw new Error(errorMsg);
                } else {
            ...
            if (data.error) {
                ...
                if (data.error.message && data.error.message.includes('execution reverted')) {
                    this.logActivity(provider, method, 'reverted', { error: 'execution reverted' });
                ...
                this.logActivity(provider, method, 'error', { error: errorMessage });
            ...
            this.logActivity(provider, method, 'success', { result: data.result ? 'data received' : 'null' });
            return data.result;
        } catch (error) {
            ...
            this.logActivity(provider, method, 'failed', { error: errorMessage });
